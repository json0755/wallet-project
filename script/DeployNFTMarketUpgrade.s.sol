...
/**
 * @title DeployNFTMarketUpgrade
 * @dev 部署NFT市场系统并实现从V1到V2的升级
 */
contract DeployNFTMarketUpgrade is Script {
    // 部署配置
    string constant NFT_NAME = "Upgradeable NFT";
    ...
    function _writeAddressesToFile() internal {
        string memory addresses = string.concat(
            "# NFT Market Deployment Addresses\n",
        ...
        vm.writeFile("nft-market-deployment-addresses.txt", addresses);
        console.log("\nDeployment addresses saved to: nft-market-deployment-addresses.txt");
    }
}